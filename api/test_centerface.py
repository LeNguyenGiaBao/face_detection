...
def camera():
    ...
    cap = cv2.VideoCapture("/home/nguyen/kientran/face/video3.mp4")
    ret, frame = cap.read()
    h, w = frame.shape[:2]
    fourcc = cv2.VideoWriter_fourcc(*'mp4v') #
    out = cv2.VideoWriter('output_praser3.mp4', fourcc, 25, (w,h), True)
...
def test_image():
    frame = cv2.imread(r'F:\\bienso.jpg')
    h, w = frame.shape[:2]
    landmarks = True
    centerface = CenterFace(model_path='./weight/face_center/centerface.onnx', landmarks=landmarks)
    if landmarks:
        dets, lms = centerface(frame, h, w, threshold=0.35)
    else:
